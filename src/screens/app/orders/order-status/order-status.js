...
const RatingSystem = ({order}) => {
  ...
  async function rateIt(rate) {
    if (rate) {
      setrating(rate);
      axiosPost(
        'order/rate',
        {order_id: order?._id, rating: rate},
        res => {
          console.log(res);
        },
        res => {
    ...
    <View>
      ...
      {/* <View style={{width: '100%'}}>
        <TouchableOpacity
          ...
          <Text
            style={{fontFamily: FONT_FAMILY.baskervilleOldFace, fontSize: 15}}>
            Submit Rating
          </Text>
        </TouchableOpacity>
      </View> */}
